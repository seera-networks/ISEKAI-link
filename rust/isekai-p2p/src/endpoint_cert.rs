//! The certificate this Endpoint presents on a peer connection: the key this
//! device holds, the request that gets it signed, and a self-signed fallback.
//!
//! **Not about video.** The listener that presents it happens to carry MJPEG
//! today and a forwarded TCP service tomorrow; what the certificate settles is
//! *which Endpoint answered*, which is the same question either way. Moved out
//! of `camera-core::tls` in plan §4.4, phase 1c-iii, so `portal-server` gets it
//! rather than a second copy of it.
//!
//! # The key is generated here and never leaves
//!
//! The relay carries the peer connection's ciphertext. While the proxy
...
//!
//! Without a proxy that issues anything, the listener presents [`dev_cert`] and
//! the initiator dials with validation disabled (dev only), so any self-signed
//! certificate works. This is not used for the proxy or Identity API TLS, which
//! are real.
//!
...
// The same re-export module the camera used, from inside the crate that owns
// it: `agent` is where this crate gathers the `isekai-p2p-core` names its API
// speaks in.
use crate::agent::{
    attest, Attestation, EndpointKey, IssuedCertificate, MasqueH3Transport, ProxyClient, ProxyError,
};
use crate::secret::write_secret;
use rcgen::KeyPair;
...
pub async fn issue(
    ...
    key: &KeyPair,
) -> anyhow::Result<Option<EndpointCert>> {
    ...
    bundle(&issued.hostname, &issued.cert_pem, key).map(Some)
}
...
pub fn load_or_generate_cert_key(path: &Path) -> anyhow::Result<KeyPair> {
    ...
    let key = KeyPair::generate_for(&rcgen::PKCS_ECDSA_P256_SHA256)
        .context("failed to generate the certificate key")?;
    ...
    write_secret(path, key.serialize_pem().as_bytes())
        .with_context(|| format!("failed to store the certificate key at {}", path.display()))?;
    ...
    read_key(path)?.with_context(|| {
        format!(
            "the certificate key vanished after writing {}",
            path.display()
        )
    })
}
    ...
    let pem = match std::fs::read_to_string(path) {
        ...
        Err(e) => {
            return Err(e).with_context(|| {
                format!("failed to read the certificate key at {}", path.display())
            })
        }
    };
    KeyPair::from_pem(&pem)
        .with_context(|| format!("failed to parse the certificate key at {}", path.display()))
        .map(Some)
}

/// A PKCS#10 certificate request for `hostname`, signed by `key`.
///
/// [`isekai_p2p_core::certificate::certificate_request`] builds it: §7.4.2 states
...
pub fn certificate_request(key: &KeyPair, hostname: &str) -> anyhow::Result<String> {
    isekai_p2p_core::certificate::certificate_request(key, hostname)
}
...
pub fn spki_sha256(key: &KeyPair) -> String {
    isekai_p2p_core::certificate::spki_sha256(key)
}
...
pub fn spki_sha256_of_certificate(der: &[u8]) -> Option<String> {
    isekai_p2p_core::certificate::spki_sha256_of_certificate(der)
}
...
pub fn bundle(hostname: &str, cert_pem: &str, key: &KeyPair) -> anyhow::Result<EndpointCert> {
...
mod key_tests {
    ...
    fn the_key_is_generated_once_and_then_reused() {
        ...
        let first = load_or_generate_cert_key(&path).expect("generate");
        let again = load_or_generate_cert_key(&path).expect("load");
    ...
    fn the_key_file_is_owner_only_from_the_start() {
        ...
        let _ = std::fs::remove_file(&path);
        load_or_generate_cert_key(&path).expect("generate");

        let mode = std::fs::metadata(&path).expect("stat").permissions().mode();
        assert_eq!(mode & 0o777, 0o600, "the certificate key must be 0600");
        let _ = std::fs::remove_file(&path);
    }
    ...
    fn an_unreadable_key_is_an_error_and_not_a_new_one() {
        ...
        let err = load_or_generate_cert_key(&path).expect_err("must not silently replace it");
        assert!(
    ...
    fn the_request_asks_for_the_name_it_was_given() {
        ...
        let _ = std::fs::remove_file(&path);
        let key = load_or_generate_cert_key(&path).expect("generate");
