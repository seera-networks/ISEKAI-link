...
1. **NAT 越えの穴あけが未検証**（§7-17）。実機 2 台が要る。**最大の未知**。
2. リレー経路の損失率（§7-19）。今回のスコープ外だが映像品質に効く。

`MaximumMtu` の件（§7-7）は Phase 6 内で解消した。
...
| 別 NAT（cone） | **未検証** | 未検証 | 実機 2 台が必要 |
| 別 NAT（symmetric） | **未検証** | 未検証 | 仕様上は不成立の見込み（リレー継続） |
| CGNAT | **未検証** | 未検証 | 同上 |

**未検証部分について正直に言うと、NAT 越えの穴あけそのものはまだ一度も検証できていない。**
成立を確認した構成はいずれも両ピアが同一ホストにあり、直接経路は LAN アドレス間である。
別ネットワーク間の確認には実機 2 台が要る。ここが残った最大の未知である。

RTT の低下は今回のマトリクスでは意味を持たない（同一ホストなのでリレー経路も直接経路も
差が小さい）。別ネットワーク間の検証時に併せて記録すること。
...
| 17 | **NAT 越えの穴あけが未検証** | 別ネットワーク間で直接経路が張れるか不明 | **未解決**: 成立を確認した構成はすべて両ピアが同一ホスト。実機 2 台での検証が必要（§6.2） |
