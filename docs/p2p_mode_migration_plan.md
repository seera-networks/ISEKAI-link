...
| 20 | ~~モバイル網の iOS 実機が P2P で一切接続できない~~ | — | **解消**: migration とは無関係の既存不具合だった。`dial_video` が映像用 FQDN を `start` に渡し、msquic がそれを**接続のワーカースレッド上のブロッキング `getaddrinfo`** で解決する（`core/connection.c` の `CxPlatDataPathResolveAddress`）。この FQDN は `A 127.0.0.1` しか持たず、DNS64 は 127.0.0.0/8 から AAAA を合成せず、rebinding 保護のあるリゾルバもループバックを返さないため、モバイル網では解決に 20〜30 秒かかる。その間ワーカーが止まるので**同じワーカーに載っているリレーレグのタイマーも動かず、レグがアイドルタイムアウトで落ちる**（レグの stats が 26 秒分まとめて出る missed-tick burst として観測）。FQDN はリレー証明書の検証名であって解決する必要がないので、`start` 前に `set_remote_addr(127.0.0.1:port)` で固定し解決を飛ばす（msquic は名前を SNI と証明書検証にだけ使う）。`msquic-async` に `set_remote_addr` を追加 |
