...
2. **フレームのキュー滞留**（新規）。`serve_frames` はクライアントごとの
   100 段チャネルへ `send().await` するため、送信が詰まると古いフレームが最大 100 枚
   （カメラ側の 100 段と合わせて最大 200 枚）滞留する。ライブ映像では捨てて最新を出すべきで、
   貯めると遅延が積み上がるだけ。実測では毎秒約 30 枚生成に対し配信は約 4 枚で、
   差はすべてキューに入るか捨てられている。
