...
1. 対称 NAT / CGNAT 環境での挙動（想定どおりリレー継続になるかの確認）。
2. ~~フレームのキュー滞留~~ — **解消**: `serve_frames` のファンアウトを
   `watch` チャネル（スロット 1 個）に変更し、latest-wins にした。
   遅いクライアントは追いつけなかったフレームを飛ばして最新を受け取る。
   配信側がブロックしなくなったので、1 つの遅いクライアントが `accept` や
   シャットダウンを止めることもなくなった。実機で体感の追従性が明確に改善。

`MaximumMtu` の件（§7-7）は Phase 6 内で、
NAT 越えの検証（§7-17）は 2026-08-01 の実機 2 台試験で解消した。

### Phase 7 — iOS ビューアへの露出  … **完了**

- `isekai-client-ffi` に経路状態と migrate 操作を露出（`PathInfo` / `PathStatus` / `FrameSink::on_path` / `ViewerSession::migrate`）。
- iOS アプリに経路表示・切替ボタン・RTT を追加。
- 実機に**ログ出力経路が無かった**ため、`tracing` を Swift 側のシンクへ橋渡し（`FrameSink::on_log` と `ClientConfig::log_filter`）。フィルタはリロード可能で、接続のたびに再適用される。
- **2026-08-01、iOS 実機で検証完了**: リレー経路で再生 → `▶ Direct` が現れて切替可能になり、切替後もフレームが流れ続け、RTT が低下することを確認。証明書検証は有効のまま（Skip TLS verification は不要）。

そこに至るまでの調査で見つけた 4 件は §7 の #20〜#23 に記録した。
...
**RTT の改善も同じ試験で確認できた: リレー経路 70 ms → 直接経路 55 ms**（約 15 ms、21% の短縮）。
低遅延化が migrate する目的そのものなので、これで効果が数字で裏づけられたことになる。

短縮幅はリレー（プロキシ）の位置と 2 拠点の位置関係で決まるため、この 15 ms は
一例であって一般的な値ではない。プロキシが両拠点から遠いほど効果は大きくなる。

**モバイル回線越しの 2 例目**（2026-08-01、片側が iPhone テザリング）:

| | リレー経路 | 直接経路 |
| --- | --- | --- |
| RTT | 153 ms | **89 ms**（−42%） |
| スループット | 168 KB/s | **255 KB/s**（+52%） |

遅延だけでなく**スループットも上がる**。リレー経路はプロキシを 2 度通るぶん
往復が長く、輻輳ウィンドウが同じでも運べる量が減るためで、
プロキシが遠い構成ほど migrate の効果は大きい。
...
| 19 | ~~リレー経路で `send_lost` が送信の約 18% に達する~~ | — | **解消**: 原因は経路ではなく MTU 設定だった。内側の映像接続は 1248 バイト固定（msquic の `QUIC_DPLPMTUD_MIN_MTU` 下限）で、CONNECT-UDP 化すると 1249 バイト。外側の下限を 1400 から落とす／外すと最大データグラム長が約 1206 になり、**フルサイズの内側パケットが 1 つも通らない**。外側の下限を必要最小値（1350、要件は約 1291）に下げて解消。実測で宣言損失 2510 中 10、うち 9 は誤検知で**実損失 1（0.04%）** |
| 20 | ~~映像用 FQDN の名前解決が msquic ワーカーを止め、リレーレグを道連れにする~~ | — | **解消**: migration とは無関係の既存不具合。`dial_video` が FQDN を `start` に渡し、msquic がそれを**接続のワーカースレッド上のブロッキング `getaddrinfo`** で解決する（`core/connection.c` の `CxPlatDataPathResolveAddress`）。この FQDN は `A 127.0.0.1` しか持たず、モバイル網では解決に 20〜30 秒かかる。その間ワーカーが止まるので**同じワーカーに載っているリレーレグのタイマーも動かず、レグがアイドルタイムアウトで落ちる**（レグの stats が 26 秒分まとめて出る missed-tick burst として観測）。FQDN は証明書の検証名であって解決する必要がないので、`start` 前に `set_remote_addr(127.0.0.1:port)` で固定し解決を飛ばす（msquic は名前を SNI と証明書検証にだけ使う）。`msquic-async` に `set_remote_addr` を追加 |
