...
#### 2.2.5 実機で判明した不具合: リレーレグのバインディング上の経路はデータを運ばない（2026-07-31）
...
| 観測 | 意味 |
| --- | --- |
| 切替後 `local/remote` が `192.168.1.223:51066 -> 192.168.1.223:49639` に変化 | 切り替え自体は成功 |
| `recv_packets` が凍結（2536 のまま 16 秒） | 切替後に 1 パケットも受信していない |
| `send_packets` が 16 秒で 1016 → 1024 | 送信も実質停止（PTO 再送のみ） |
| `rtt_us=333000` で固定 | 333 ms は msquic の新規パス初期推定値。**新パス上のパケットが一度も ACK されていない** |
| サーバは `127.0.0.1` の経路のまま、直後に送信がブロック | サーバは移行できず ACK 待ちで停止 |
| `send_path_mtu=1248` が前後で不変 | MTU は無関係 |

最終的に `QUIC_STATUS_CONNECTION_TIMEOUT` で接続が落ちる。

##### 切り分け

クライアント側で `add_candidate_addr` を呼ばない（＝リレーレグのバインディングを候補として出さない）
ようにすると、**映像は流れ続けた**。この場合 msquic は直接経路のために**自前のバインディングを開く**。

したがって原因は **「リレーレグのバインディング上に開かれた経路」そのもの**にある。

- Phase 0 スパイクではこれが再現しない。スパイクのリレーレグ代役は
  **プレーンな QUIC 接続**（`set_share_binding` + `set_unconnected_socket` + `set_local_addr`）であり、
  実アプリのレグは `H3MsQuicAsyncConnector` が張る **MASQUE H3 接続**である。
- スパイクは 2 プロセス構成・本番コード経路（`serve_frames_with` / `receive_frames_with`）・
  実アプリと同じ候補順序でも実機で PASS する。バインディング共有そのものは動く。

##### 除外済みの要因（すべて実機または Windows CI で確認）

ファイアウォール、フレームサイズ（30 KB）、候補が 2 つあること、候補の順序、
2 プロセスであること、`MaximumMtu(1200)`、`DestCidUpdateIdleTimeoutMs`、
`serve_frames_with` / `receive_frames_with` の実装。

##### 回避策にはならない

`ISEKAI_MIGRATION_NO_CANDIDATE` が効くのは、**両ピアが同一 LAN にいて直接経路が NAT を通らない**
場合に限られる。msquic が自前で開いたバインディングにはプロキシが観測した NAT マッピングが無く、
別ネットワーク間では穴あけできない。案C がリレーレグのバインディングを名指しするのは、
**そのバインディングのマッピングだけが観測済みだから**である。

##### 両側のカウンタで確定したこと（2026-07-31 追記）

サーバ側の統計をタイマー方式に直して再取得したところ、次が判明した。

- **サーバも移行に追随する。** クライアントの `activate_path` から約 0.8 秒後、
  サーバの映像接続も `127.0.0.1:64980 -> 127.0.0.1:60215` から
  `192.168.1.223:58952 -> 192.168.1.223:52417` へ移り、フォールバック後は同様に戻る。
  片側だけ取り残される問題ではない。
- **直接経路では双方向とも実質無通信。** サーバ側 6 秒で送信 +8 / 受信 +2、
  クライアント側も送信 +7 / 受信は最初の 1 秒で +12 のあと停止。
- **サーバは直接経路で RTT 14.4 ms を計測しているが、クライアントは 333 ms（初期値）のまま。**
  同一ホスト上の LAN アドレス同士なら RTT は 1 ms 未満のはずで、
  **14 ms はサーバのリレーレグからプロキシまでの RTT（13〜17 ms）とほぼ一致する。**
  経路検証のパケットがホスト内で直接やり取りされていない可能性を示す。

つまり **`PathValidated` は上がるが、その経路は実際には双方向に使えていない**。

##### 修正の方向: 生きているレグのバインディング上に経路を作らない

`ISEKAI_MIGRATION_NO_CANDIDATE=1`（候補を出さず、msquic に自前のバインディングを開かせる）では
**映像が流れ続ける**ことが実機で確認できている。したがって:

- msquic **自身が開いたバインディング**上の直接経路は動く
- **生きている MASQUE レグと共有するバインディング**上の直接経路は動かない

そこで案C を次のように改める（Direct モードの «probe & drop» と同じ形）:

1. プロキシへ**短命の QUIC 接続**を別途張り、`NotifyObservedAddress` で
   そのバインディングの `(local, observed)` を得る
2. その接続を**閉じる**（アドレスが空く）
3. 映像接続の候補として `add_candidate_addr(local, observed)` を渡す

こうすると直接経路は **msquic が自前で開くバインディング**上に載り、かつ
**NAT マッピングは観測済み**という両方の条件を満たす。
リレーレグのバインディングには一切触れない。

懸念は NAT マッピングの失効だが、穴あけのトラフィック自体がマッピングを維持するため、
プローブから dial までの数百 ms であれば実用上問題にならないと見込む（要検証）。

##### 根本原因（msquic ETW トレースで確定, 2026-08-01）

受信側のバインディングでパケットが破棄されていた。

```
[bind][0x17234B5B5A0] DROP packet Dst=192.168.1.223:59176 Src=192.168.1.223:55052
    Dest CID=0x0CF2A1F7F9DE78131F Reason=No matching client connection.
```

- 送信は正常に行われている（datapath の送信エラーは無し）
- パケットは **ホスト内を直接届いている**（Src/Dst とも LAN アドレス）
- しかし **バインディングの接続テーブルに宛先 CID が無く**、そこで捨てられる

つまり `add_bound_addr` / `add_candidate_addr` は、そのアドレスを**送信元として**接続に結び付けるが、
**そのバインディングの受信ルックアップに接続の CID を登録していない**。
結果として送信だけが通り、受信は全て破棄される。観測された
「送信カウンタは微増・受信カウンタは完全に凍結」と一致する。

> 訂正: 先に「サーバの直接経路 RTT が 14 ms なのでパケットがホスト内を通っていない可能性がある」と
> 記したが、これは誤りだった。パケットは届いており、届いた先で捨てられていた。

これは **seera-msquic 側の不具合**である。生きている接続と共有するバインディング上に
`add_bound_addr` / `add_candidate_addr` で経路を追加したとき、
その接続が当該バインディングの接続ルックアップに登録されない。

##### 訂正: 「修正の方向」は誤った前提に基づいていた（2026-08-01）

上の §2.2.5「修正の方向」と、それに基づく «probe & drop» 実装は、
**`ISEKAI_MIGRATION_NO_CANDIDATE=1` で映像が流れ続けた**という報告を
「msquic が自前で開いたバインディング上の直接経路なら動く」と解釈したものだった。

これは**誤読**である。この設定では Migrate ボタンがグレーのままになる —
つまり `DirectValidated` が一度も上がらず、**直接経路は張られていない**。
映像が流れ続けたのは**リレー経路のまま**だったからで、migration の成功例ではなかった。

したがって、

- 「msquic 自前のバインディングなら動く」という結論は**根拠を失う**
- «probe & drop»（空いたアドレスを候補にする）は Windows で **FAIL**。実際に試して確認済み
...
| --- | --- | --- |
| 候補にレグのアドレスを渡す（元の案C） | 上がる | **不通**（ETW: `No matching client connection`） |
| 候補にプローブで解放したアドレスを渡す（probe & drop） | 上がる | **不通** |
| 候補を出さない（`NO_CANDIDATE`） | **上がらない** | （migration 自体が起きない） |
| 2 プロセススパイク（本番コード経路・同一設定） | 上がる | **疎通する** |

**スパイクは同じ `prepare_for_migration` 相当の設定で Windows でも疎通する。**
実アプリとの差分は依然として **実 MASQUE リレーレグ**であり、
probe & drop はレグの*アドレス*を避けただけで、レグ自体は同じプロセス・同じ registration に
生きたまま残っている。そこは切り離せていない。

##### 次に確かめること

1. **リレーレグが QMUX にフォールバックしていないか。**
   `H3MsQuicAsyncConnector::connect` は `start` が失敗すると `Connection::new_qmux` で
   **新しい接続を作り直す**が、そこには `set_share_binding` /
   `set_unconnected_socket` / `set_local_addr` が適用されない
   （`submodules/tonic-h3` の `a68d128`）。フォールバックした場合 `is_unconnected` は黙って無視される。
   これは経路の有無に関わらず connector 側の不具合であり、まず潰すべき。
2. 1 で説明がつかない場合、Windows で msquic の ETW トレース（`quic.etl`）を取得し、
   共有バインディング上の送信が OS レベルで失敗しているのかを確認する。

##### `MaximumMtu` が効いていない（別件）

`send_path_mtu` は一貫して **1248** で、`video_client_config` の `set_MaximumMtu(1200)` は
