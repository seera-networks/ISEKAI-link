...
アプリ側は §2.2.5「修正の方向」の «probe & drop» に切り替えた。
プロキシへ短命の接続を張って `(local, observed)` を得てから閉じ、
**空いたアドレス**を候補として渡す。直接経路は msquic が自前で開くバインディングに載るので、
上記の不具合を踏まない。実プロキシに対する `relay_migration_e2e` で確認済み。

msquic 側の修正が入れば、リレーレグのバインディングを直接使う形（元の案C）に戻せる。
そちらのほうがプローブ 1 往復と NAT マッピング維持の前提を省けるので望ましい。
