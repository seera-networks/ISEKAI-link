...
| 13 | **Grant が Listener に紐づいていると、アプリを再起動するたびにペアリングが死ぬ** | 実機で発生。カメラを閉じて開き直すと、クライアントの一覧に新しいカメラが出ない。1 時間以内は死んだ古いリスナが出て、掃引後は空になる | **解消**: Grant のキーを `(owner_endpoint, allowed_endpoint, protocol)` に移した（サーバ仕様 §8.8、ISEKAI-link-server #203）。Endpoint 鍵は永続なのでペアリングも永続。合わせて閉じるときに Listener を `DELETE` するようにした — 受け付けていないリスナが一覧に残らない |
...
- Listener を作り直しても **grant が生き残る**こと、および削除が grant を道連れに
  しないこと（#13）
