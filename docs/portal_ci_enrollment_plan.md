...
5. **エラーの一様性を潰さない。** `403 enrollment-key-invalid` は未知・期限切れ・失効・
...
6. **`Retry-After` を返り値に載せる。** `IdentityError::Api` に `retry_after: Option<Duration>`
...
- ジョブの最後に `kill` する。プロセスが消えればトークン更新が止まり、
  `endpoint_idle_ttl` の掃引が Endpoint を失効させて枠が空く（§4.6）。
...
### 4.6 枠は「同時実行数」ではなく「同時に枠を占めている数」

Identity 仕様 §8.8.11 の目安。

```text
max_live_endpoints ≈ 並列度 × (ジョブ実行時間 + endpoint_idle_ttl) / ジョブ間隔
```

既定（枠 4 / `idle_ttl` 3,600 / 掃引 300）で 5 分のジョブを並列 4 で回すと、
**4 枠が約 1 時間占有される。** 詰まるのは「同時 5 本目」ではなく次に来たジョブで、
約 55 分にわたって `429 enrollment-slots-exhausted` が返る。

**枠を増やす前に `endpoint_idle_ttl` を縮める。** 生きているランナーは 4 分ごとに
トークンを更新するので、`idle_ttl` は 1,800 で足りる。§6 の推奨値はこれに従う。

**ここに 1 つ穴がある。** ジョブの終わりに Endpoint を明示的に失効させられれば枠は
即座に空くが、§8.7 の失効 API は Auth0 AT を要求するので、CI ランナーには呼べない。
掃引を待つしかない。§7 の未解決 1。
...
| 設定 | 推奨 | 理由 |
| --- | --- | --- |
| `endpoint_idle_ttl` | 1,800 | トークン TTL の 2〜3 倍。§4.6 |
| `max_live_endpoints` | §4.6 の式で算出。既定 4 は 5 分ジョブ×並列 4 で足りない | |
...
1. **ジョブ終了時に枠を返せない。** §8.7 の失効は Auth0 AT を要求するので、CI ランナーは
   自分の Endpoint を失効させられない。掃引を待つしかなく、そのぶん枠を厚く見積もる
   必要がある（§4.6）。自然な解は §8.8.7 と同じ形の「Enrollment Key による自己失効」だが、
   それは仕様の追補になる。Identity 仕様 §8.8.12 に近い論点はあるが、この形では挙がって
   いない。**上流に上げる価値がある。**
2. **`refresh/challenge` の要求が assertion を要るのかどうか。** §8.8.7 は「この 2 つの
   エンドポイントは `enrollment_key` をボディに載せた要求を受理する」と書き、
   例のボディは更新側のものである。Challenge 側にも assertion が要るなら鋳造が 1 回増える。
   §8.8.4 が「challenge では binding を検証しない」としているのと同じ判断なら要らない。
   **フェーズ 1 の前に確認する。**
