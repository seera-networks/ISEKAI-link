...
| **3b** | The sockets: a session per (source, service), one UDP socket each, the idle sweep, and the datagram pump both ways | **done** — `portal-core::udp`, and **forwards UDP over a real proxy**. Loopback covers the round trip, two sources not being crossed, a payload at the limit, the sweep and the refusal parity; the size bound above is the one thing hardware cannot make go away |
