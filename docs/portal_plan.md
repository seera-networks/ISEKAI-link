...
| **6** | Pairing: portal authorises with a Grant rather than a hand-carried capability, as the camera apps do | a peer reconnects, and survives the server restarting, without the operator |
| | **done** — `--pair` on both sides. The mechanism was already built and portal was using the other one: spec §13.3 keeps Grant and Capability apart as *standing permission* and *one delegation*, and portal had spent five phases using the second for the first's job. §8.8 keeps Listener out of a Grant's key, so the client looks the current listener up by the server's Endpoint ID | #166 |
| **7** | Distribution: an archive somebody can download and run | `./portal-server` starts on a machine that has never built this |
| | **done** — `scripts/bundle-apps.{sh,ps1}` and a release job in `portal.yml`. The camera apps already solved it; the script was theirs and knew their two binaries by name, so it takes the app list now and both workflows share it. Tag-only here, because portal's ordinary CI is a four-minute test job and the camera's every-push runs are what keep the machinery exercised | #167 |
| **8** | Auth0 tokens that refresh, so a server can be left running | a session outlives the access token it started with |
| | **done** — `portal-* --login` runs the device flow headlessly and saves tokens beside the Endpoint key. `P2pConfig::auth0` was the seam and portal had been passing `None`; the camera apps drive the same flow from a GUI. `--auth0-token` stays for scripts and now says in the log that it cannot be refreshed | |
| **9** | Use what the transport can now tell us: per-path statistics, and the datagram state | a dead preferred path is detected locally |
| | **done** — `portal-core::path` reports `get_path_statistics` rather than `get_stats`, which described the *first* path while the log said `direct`, and falls back to the relay when the preferred path holds data with no acknowledgement for ten seconds. `DatagramStateChanged` says once whether the peer takes datagrams at all. Closes #172 and the third item of #165 | not verified on hardware — see #165 |
| | **done** — `docs/portal.md`, linked from the top-level README. `--help` carries the catalogue format and the exchange that gets a peer connected; `--example-config` writes a starter file that `config`'s own tests parse. Both binaries default `--key`, `--config` and the log filter, the last of which was the real defect: `from_default_env()` defaults to `ERROR`, so every `warn!` in the forwarding went nowhere — and `fmt()` writes to stdout, in among the ids the operator is meant to copy | writing the page found two things the page could not paper over: #166 and #167, both since fixed |
